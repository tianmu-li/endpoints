...
| **Dataset Manager**    | `src/inference_endpoint/dataset_manager/`                                               | Loads JSONL, HuggingFace, CSV, JSON, Parquet datasets. `Dataset` base class with `load_sample()`/`num_samples()` interface. Datasets with `ACCURACY_ONLY = True` (e.g. `SWEBench`) may only be used as accuracy datasets; using them as the performance dataset raises `InputValidationError`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **SWE-bench**          | `src/inference_endpoint/dataset_manager/predefined/swe_bench/`, `evaluation/scoring.py` | `SWEBench` predefined dataset (HuggingFace `princeton-nlp/SWE-bench_Verified` or `_Lite`; `ACCURACY_ONLY=True`). `SWEBenchScorer` sets `SKIP_ENDPOINT_PHASE=True` and bypasses the built-in accuracy phase entirely: it calls `mini-extra swebench` then `swebench.harness.run_evaluation` via `uv run --project examples/10_Agentic_Inference/accuracy/`. `preflight()` validates uv/Docker/mini-extra/swebench, calls `_apply_tools_patch()` to overwrite `actions_toolcall.py` and `litellm_model.py` in the minisweagent install with replacement files shipped in `examples/10_Agentic_Inference/accuracy/`, and pre-pulls required Docker images before the benchmark starts. The replacements add `finish` (auto-extracts `git diff HEAD` on submit, eliminating the manual patch.txt ceremony) and `str_replace_editor` (exact-match atomic file edits) tools to minisweagent 2.3.0. `external_sample_count()` surfaces the configured instance count to the framework's sample-count log. |
...
│   └── predefined/            # Built-in datasets (aime25, cnndailymail, gpqa, swe_bench, etc.)
├── metrics/
